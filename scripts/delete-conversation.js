/**
 * Delete a specific conversation
 * Usage: node scripts/delete-conversation.js <senderId>
 */
const { Firestore } = require('@google-cloud/firestore');
const fs = require('fs');
const path = require('path');

const senderId = process.argv[2];
if (!senderId) {
  console.error('Usage: node scripts/delete-conversation.js <senderId>');
  process.exit(1);
}

const envPath = path.join(__dirname, '..', '.env.prod');
if (fs.existsSync(envPath)) {
  fs.readFileSync(envPath, 'utf8').split('\n').forEach(line => {
    if (!line.trim() || line.startsWith('#')) return;
    const idx = line.indexOf('=');
    if (idx > 0) {
      let key = line.substring(0, idx).trim();
      let val = line.substring(idx + 1).trim();
      if (val.startsWith('"') && val.endsWith('"')) val = val.slice(1, -1);
      if (!process.env[key]) process.env[key] = val;
    }
  });
}

let privateKey = process.env.GOOGLE_CLOUD_PRIVATE_KEY || '';
privateKey = privateKey.replace(/\\n/g, '\n').trim();

let clientEmail = process.env.GOOGLE_CLOUD_CLIENT_EMAIL || '';
clientEmail = clientEmail.replace(/\\n/g, '').trim();

const db = new Firestore({
  projectId: 'bebias-wp-db-handler',
  credentials: { client_email: clientEmail, private_key: privateKey },
});

async function deleteUser() {
  console.log('Deleting data for sender:', senderId);

  // Delete conversation
  await db.collection('conversations').doc(senderId).delete();
  console.log('✅ Deleted conversation');

  // Delete rate limits
  try {
    await db.collection('rateLimits').doc(senderId).delete();
    console.log('✅ Deleted rate limits');
  } catch (e) {}

  // Delete meta messages
  try {
    await db.collection('metaMessages').doc(senderId).delete();
    console.log('✅ Deleted meta messages');
  } catch (e) {}

  console.log('\n✅ Done! User data deleted.');
}

deleteUser().then(() => process.exit(0)).catch(e => {
  console.error('Error:', e.message);
  process.exit(1);
});
