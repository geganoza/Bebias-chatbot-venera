...
// Message batching variables
let messageQueue = {};  // Store pending messages per user
let batchTimeouts = {};  // Store batch timers per user
const BATCH_DELAY = 2000;  // Wait 2 seconds to collect messages

// Initialize
document.addEventListener('DOMContentLoaded', () => {
    console.log('Test simulator loaded!');
...
function initializeTestUsers() {
    ...
    if (!container) {
        console.error('testUsers container not found!');
        return;
    }
...
function setupEventListeners() {
    ...
    // Test batch button - sends 3 quick messages
    document.getElementById('testBatchBtn').onclick = () => {
        if (currentUser) {
            const testMessages = ['გამარჯობა', 'რა ქუდები გაქვთ?', 'ფოტო გაქვს?'];
            testMessages.forEach((msg, index) => {
                setTimeout(() => {
                    messageInput.value = msg;
                    sendMessage();
                }, index * 300);  // 300ms between messages
            });
        }
    };
}
...
function sendMessage() {
    ...
    const batchingEnabled = document.getElementById('enableBatching').checked;

    if (batchingEnabled) {
        // Add to message queue for batching
        addToMessageBatch(text);
    } else {
        // Send immediately without batching
        showTyping();
        getBotResponse(text, 1);
    }
}
 ...
 * Waits BATCH_DELAY ms to collect multiple messages before sending
 */
function addToMessageBatch(text) {
    if (!currentUser) return;
    ...
    messageQueue[userId].push(text);
    console.log(`📦 Added to batch (${messageQueue[userId].length} messages queued)`);

    // Clear existing timeout
    if (batchTimeouts[userId]) {
        clearTimeout(batchTimeouts[userId]);
    }
    ...
    batchTimeouts[userId] = setTimeout(() => {
        processBatch(userId);
    }, BATCH_DELAY);

    console.log(`⏱️ Batch timer set for ${BATCH_DELAY}ms`);
}

/**
 * Process batched messages
 * Combines multiple messages and sends to API
 */
async function processBatch(userId) {
    if (!messageQueue[userId] || messageQueue[userId].length === 0) {
        hideTyping();
        return;
    }

    const batchedMessages = [...messageQueue[userId]];
    messageQueue[userId] = [];  // Clear queue

    console.log(`🚀 Processing batch: ${batchedMessages.length} messages`);
    console.log(`📝 Messages: ${batchedMessages.join(' | ')}`);

    // Combine messages with period separator for clarity
    const combinedMessage = batchedMessages.join('. ');

    // Send combined message to API
    await getBotResponse(combinedMessage, batchedMessages.length);
}

async function getBotResponse(userMessage, batchCount = 1) {
    try {
        const useTestInstructions = document.getElementById('useTestInstructions').checked;
        const debugMode = document.getElementById('debugMode').checked;

        if (batchCount > 1) {
            console.log(`🔀 BATCHED REQUEST: ${batchCount} messages combined`);
        }
        ...
        // Convert messages to OpenAI format (exclude the messages we just added)
        // Since we might have added multiple messages, exclude batchCount messages
        ...
        console.log('Sending to API with history:', conversationHistory.length, 'messages');
